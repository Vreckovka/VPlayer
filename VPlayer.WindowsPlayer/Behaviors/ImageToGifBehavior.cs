...
namespace VPlayer.WindowsPlayer.Behaviors
{
  ...
  public static class ImageBehavior
  {
    ...
    private static void AnimateImage(Image imageControl, BitmapDecoder decoder, BitmapSource source)
    {
    ...
    private static void InitAnimationOrImage(Image imageControl)
    {
      ...
      if (source != null)
      {
        var decoder = GetDecoder(source);
        if (decoder != null && decoder.Frames.Count > 1 && decoder is GifBitmapDecoder bitmapDecoder)
        {
          AnimateImage(imageControl, bitmapDecoder, source);
        }
        else if(decoder != null)
        {
          decoder.DownloadCompleted += (sender, e) =>
          {
            if (sender is BitmapDecoder bitmap )
            {
              AnimateImage(imageControl, bitmap, source);
            }
          };
        }
      }
    ...
    private static void Decoder_DownloadCompleted(object sender, EventArgs e)
    {
    }
