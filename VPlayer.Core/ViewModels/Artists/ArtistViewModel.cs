...
namespace VPlayer.Core.ViewModels.Artists
{
  ...
  public class ArtistViewModel : PlayableViewModelWithThumbnail<SongInPlayListViewModel, Artist>
  {
    ...
    public override string BottomText => "Number of albums: " + Model.Albums?.Count.ToString();
    ...
    {
      var songs = storage.GetRepository<Artist>().Include(x => x.Albums).ThenInclude(x => x.Songs).ThenInclude(x => x.SoundItem).Where(x => x.Id == Model.Id).ToList();
